...
class gradient_ascent: #TODO: handle non seen tags!!!!!
    ...
    def log_of_numerator(self):
        ...
        for row in range(self.feature_maker.number_of_sentences):
            sum_of_numerator+=vector_multiplication(self.vector_v,self.feature_maker.feature_matrix[row])
        return sum_of_numerator

    def log_of_denominator(self):
        total_sum = 0
        for trigram in self.feature_maker.expected_feature_matrix_index:
            ...
            total_sum+=np.log(self.sum_of_exponential_permutations(matrix))
        return total_sum


    def sum_of_exponential_permutations(self,matrix):
        feature_sum = 0
    ...
    def regularized_log_likelihood(self,sentences,trigrams,param_index):
        return self.log_of_denominator()-self.log_of_numerator()+float(float(self.lambda_value)/2)*vector_multiplication(self.vector_v,self.vector_v)


    def gradient_of_log_likelihood(self):
        for trigram in self.feature_maker.expected_feature_matrix_index:
            matrix = self.feature_maker.expected_feature_matrix_index[trigram]
            sum_of_exponentias = self.sum_of_exponential_permutations(matrix)
            expected_sum = csr_matrix((1,self.number_of_dimensions),dtype=float).toarray()
            for row in range(matrix.get_shape()[0]):
                feature = matrix.getrow(row).toarray()
                expected_feature = feature.multiply(self.probability_calculation(feature,sum_of_exponentias))
                expected_sum+=expected_feature
        return expected_sum - self.sum_of_feature_vector + self.vector_v.multiply(self.lambda_value)




    def probability_calculation(self,feature,sum_of_exp_prem):#TODO : handle non seen words
        numerator = np.exp(self.vector_v,feature)
